...
export function saveStory(transaction, story) {
	if (!story.id) {
...
export function deleteStory(transaction, story) {
	if (!story.id) {
		throw new Error('Story has no id');
	}
...
export function savePassage(transaction, passage) {
	if (!passage.id) {
...
export function deletePassage(transaction, passage) {
	if (!passage.id) {
		throw new Error('Passage has no id');
	}
...
export function deletePassageById(transaction, id) {
	transaction.passageIds = remove(transaction.passageIds, id);
