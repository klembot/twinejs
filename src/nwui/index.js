...
var nwui = {
	...
	initialize: function() {
		...
		try {
			...
			if (process.platform == 'darwin') {
				...
				new nwui.gui.MenuItem({ type: 'separator' }), 0
				);
		...
		if (e.which == 68 && e.shiftKey && e.altKey && e.ctrlKey) {
			...
			**/
			...
			if (! nwui.fs.existsSync(docPath)) {
				...
				// Regardless, this must have a single forward slash (/)
		...
		this.on('change', _.throttle(function() {
			...
			var isLastUpdatedOnly = ! _.some(changedAttributes, function(key) {
	...
	saveStoryFile: function(story) {
		try {
			...
			notify(
			locale.say(
			'An error occurred while saving your story (%s).', e.message
			),
			'danger'
			);
			throw e;
	...
	deleteStoryFile: function(story) {
		try {
			...
			notify(
			locale.say(
			'An error occurred while deleting your story (%s).', e.message
			),
			'danger'
			);
		} finally {
