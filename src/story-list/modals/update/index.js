	...
	check: function ()
	{
		...
		if (now > lastUpdateCheckPref.get('value') + this.CHECK_INTERVAL)
		{
			TwineApp.checkForUpdate(lastUpdateSeenPref.get('value'), function (data)
			{
				...
				/* L10n: %1$s will have a version number, i.e. 2.0.6, interpolated into it. */
				var message = locale.say('A new version of Twine, %1$s, has been released.', data.version);

				confirm({
					confirmLabel: '<i class="fa fa-download"></i> ' + locale.say('Download'),
