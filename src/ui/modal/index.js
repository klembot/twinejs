...
module.exports =
{
	...
	open: function (options)
	{
		...
		this.$modal.addClass('fadeIn').one('animationend', function()
		{
			this.$modal.trigger('modalOpen.twineui');
		}.bind(this))
		.find('#modal').addClass('appear');
	...
	close: function()
	{
		...
		this.$modal.removeClass('fadeIn').addClass('fadeOut')
		.one('animationend', function()
		{
			this.$modal.trigger('modalClose.twineui').off('.twineui').remove();
			$('body').removeClass('modalOpen');
			$(document).off('.twineui');
		}.bind(this)).find('#modal').removeClass('appear').addClass('disappear');
	}
};
