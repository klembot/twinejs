...
describe('story actions module', () => {
	...
	test(
        'ensures a story created with createStory() always has a story format set',
        () => {
	...
	test('dispatches a DUPLICATE_STORY mutation with deleteStory()', () => {
		actions.duplicateStory(store, fakeId, 'Another Name');
		expect(store.dispatch.calledOnce).toBe(true);
		expect(store.dispatch.calledWith('DUPLICATE_STORY', fakeId, 'Another Name')).toBe(true);
	});
	...
	test('dispatches an UPDATE_STORY mutation with updateStory()', () => {
		...
		expect(store.dispatch.calledOnce).toBe(true);
		expect(store.dispatch.calledWith('UPDATE_STORY', fakeId, props)).toBe(true);
	});
	...
	test(
        'coerces old SugarCube references to their correct versions with repairStories()',
        () => {
            let storiesStore = {
                dispatch: spy(),
                state: {
	...
	test(
        'leaves story format versions alone if the story is already up-to-date',
        () => {
            let storiesStore = {
                dispatch: spy(),
                state: {
