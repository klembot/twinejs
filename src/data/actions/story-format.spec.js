...
describe('story format actions module', () => {
	...
	it('creates built-in formats with repairFormats()', () => {
		...
		expect(created['Harlowe-3.2.2']).to.exist;
		expect(created['Harlowe-3.2.2'].url).to.equal(
			'story-formats/harlowe-3.2.2/format.js'
		);
		expect(created['Harlowe-3.2.2'].userAdded).to.be.false;
		expect(created['Harlowe-2.1.0']).to.exist;
		...
		expect(created['SugarCube-2.35.0']).to.exist;
		expect(created['SugarCube-2.35.0'].url).to.equal(
			'story-formats/sugarcube-2.35.0/format.js'
		);
		expect(created['SugarCube-2.35.0'].userAdded).to.be.false;
	});
