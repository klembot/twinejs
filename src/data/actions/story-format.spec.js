...
describe('story format actions module', () => {
	...
	it('creates built-in formats with repairFormats()', () => {
		...
		expect(created['SugarCube-2.36.0']).to.exist;
		expect(created['SugarCube-2.36.0'].url).to.equal(
			'story-formats/sugarcube-2.36.0/format.js'
		);
		expect(created['SugarCube-2.36.0'].userAdded).to.be.false;
	});
