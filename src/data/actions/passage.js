...
import {displace, intersects} from '../../common/rect';

export function createPassage({dispatch}, storyId, props) {
	dispatch('CREATE_PASSAGE_IN_STORY', storyId, props);
}

export function updatePassage({dispatch}, storyId, passageId, props) {
	dispatch('UPDATE_PASSAGE_IN_STORY', storyId, passageId, props);
}

export function deletePassage({dispatch}, storyId, passageId) {
	dispatch('DELETE_PASSAGE_IN_STORY', storyId, passageId);
}

export function selectPassages(store, storyId, filter) {
...
export function positionPassage(store, storyId, passageId, gridSize, filter) {
	...
	*/
	...
	story.passages.forEach(other => {
		...
		if (intersects(otherRect, passageRect)) {
			displace(passageRect, otherRect, displacementDistance);
		}
	});
...
export function changeLinksInStory(store, storyId, oldName, newName) {
