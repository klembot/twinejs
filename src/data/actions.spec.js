...
describe('actions data module', () => {
	...
	it('creates built-in formats with repairFormats()', () => {
		...
		expect(created['SugarCube-2.13.0']).to.exist;
		expect(created['SugarCube-2.13.0'].url).to.equal('story-formats/sugarcube-2.13.0/format.js');
		expect(created['SugarCube-2.13.0'].userAdded).to.be.false;	
	});
