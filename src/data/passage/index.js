...
var Passage = Backbone.Model.extend({
  ...
  validate: function(attrs, options) {
    ...
    var passages = data.passagesForStory(data.storyForPassage(this));
    var passageAlreadyExist = passages.find(function(passage) {
      return (attrs.id != passage.id &&
      attrs.name.toLowerCase() == passage.get('name').toLowerCase());
    });

    if (passageAlreadyExist) {
      return locale.say('There is already a passage named "%s."' +
          ' Please give this one a unique name.', attrs.name);
    }
  },
  ...
  **/
  ...
  /**
    ...
    @method links
    @param {Boolean} [internalOnly] only return internal links?
  ...
  */
  ...
  links: function(internalOnly) {
    ...
    // `[[display text->link]]` or `[[link<-display text]]` format.
    // They may also have a setter component, in`[[display text->link][...]]` or
    // `[[link<-display text][...]]` format.
    // This regexp will interpret the rightmost `->` and the leftmost `<-` as
    // the divider.
    var arrowLinksRe =
        /\[\[(?:([^\]]*)\->|([^\]]*?)<\-)([^\]]*)(?:\]\[.*?)?\]\]/g;
  ...
  **/
  ...
  /**
    Returns the total number of string matches in this passage
  ...
  **/
  ...
  /**
    Performs a regexp replacement on this passage's text,
  ...
  **/
  ...
  /**
    ...
    @method publish
    @param {Number} id numeric id to assign to the passage,
                       *not* this one's DB id
