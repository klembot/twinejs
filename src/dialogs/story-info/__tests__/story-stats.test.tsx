...
describe('<StoryInfoDialogStats>', () => {
	...
	it('shows a character count for the story', () => {
		...
		const row = screen.getByText('dialogs.storyInfo.stats.characters')
			.parentNode;
	...
	it('shows a word count for the story', () => {
		...
		const row = screen.getByText('dialogs.storyInfo.stats.words').parentNode;
	...
	it('shows a passage count for the story', () => {
		...
		const row = screen.getByText('dialogs.storyInfo.stats.passages').parentNode;
	...
	it('shows a distinct link count for the story', () => {
		...
		const row = screen.getByText('dialogs.storyInfo.stats.links').parentNode;
	...
	it('shows a broken link count for the story', () => {
		...
		const row = screen.getByText('dialogs.storyInfo.stats.brokenLinks')
			.parentNode;
	...
	it('shows the time the story was last updated', () => {
		renderComponent();
		expect(
			screen.getByText('dialogs.storyInfo.stats.lastUpdate')
		).toBeInTheDocument();
	});
	...
	it("shows the story's IFID", () => {
		renderComponent();
		expect(
			screen.getByText('dialogs.storyInfo.stats.ifid')
		).toBeInTheDocument();
	});
	...
	it('shows a link that explains what an IFID is', () => {
		renderComponent();
		expect(
			screen.getByText('dialogs.storyInfo.stats.ifidExplanation')
