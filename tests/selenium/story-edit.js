...
test.describe('StoryEditView', function() {
	...
	test.it('Updates links when passages are renamed', function() {
		...
		dr.actions().doubleClick(dr.findElement({ css: '.passages .passage:nth-of-type(2)' })).perform();

		var passageName = dr.findElement({ css: '#passageEditModal input.passageName'});
		passageName.clear();
		passageName.sendKeys('2 linked passage');
		dr.findElement({ css: '#passageEditModal .close' }).click();
		dr.wait(until.elementIsNotVisible(modal));

		assert(dr.findElement({ css: '.passages .passage:first-child .excerpt'}).getText())
	...
	test.it('Renames a story via a dialog', function() {
		...
		dr.findElement({ css: '#renameStoryModal input.storyName' }).sendKeys('This is different');
	...
	test.it('Numbers new passages to avoid name conflicts', function() {
		...
		dr.findElement({ css: '.toolbar .addPassage' }).click();
		dr.wait(until.elementLocated({ css: '.passages .passage:nth-of-type(3) .title' }));
	...
	test.it('Shows accurate story statistics', function() {
		...
		dr.actions().doubleClick(dr.findElement({ css: '.passages .passage' })).perform();
		dr.wait(until.elementIsVisible(modal));
		setCMText(dr, '#passageEditModal', '[[red]] [[green]] [[blue]] The quick brown fox jumps over the lazy dog.');
		...
		dr.actions().mouseMove(passage).sendKeys(key.SHIFT).perform();
		dr.findElement({ css: '.passages .passage:nth-of-type(2) .delete' }).click();
		dr.wait(until.stalenessOf(passage));
