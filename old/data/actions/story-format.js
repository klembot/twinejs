...
export function createFormat({dispatch}, props) {
	dispatch('CREATE_FORMAT', props);
}

export function updateFormat({dispatch}, id, props) {
	dispatch('UPDATE_FORMAT', id, props);
}

export function deleteFormat({dispatch}, id) {
	dispatch('DELETE_FORMAT', id);
}

export function createFormatFromUrl(store, url) {
...
export function loadFormat(store, name, version) {
	/*
	...
	*/
	...
	return new Promise((resolve, reject) => {
		...
		jsonp(format.url, {name: 'storyFormat', timeout: 2000}, (err, data) => {
			if (err) {
				reject(err);
				return;
			}
...
export function repairFormats(store) {
	/*
	...
	*/
	...
	if (typeof store.state.pref.defaultFormat !== 'object') {
		setPref(store, 'defaultFormat', {name: 'Harlowe', version: '3.0.2'});
	}
